...
Our goal is to create a framework to further generalize Distributed Hash Tables (DHTs) so they can be used for distributed computing.
...
Many P2P applications use DHTs to address theses challenges, since DHTs are designed with these problems in mind.
...
The primary application of what I am doing is to create 

My work could be used for a quick and intuitive way to solve embarrassingly parallel problems, such as:
\begin{itemize}
	...
	\item Random forests.
\end{itemize}
Essentially, any process that is easily \texttt{map}pable to individual nodes, we want to do it. 
