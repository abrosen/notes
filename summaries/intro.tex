...
FOO
Our goal is to create a framework to further generalize Distributed Hash Tables so they can be used for distributed computing.
...
Distributed Hash Tables (DHTs) provide all of these qualities.
...
Rather than keys being assigned to some data, we can assign keys to tasks and automatically distribute those tasks to the responsible node.s
...
The remainder of the proposal will then discuss the work I plan on doing to demonstrate the viability of using DHTs for distributed computing.
...
% Why should you care?
\section{Why You Should Be Excited}

Distributed Hash Tables are the backbone for many P2P systems.
\subsection{Applications}

Distributed Hash Tables have numerous applications:
...
We can see  from this list that DHTs are %hot

The applications of what I'm doing:

\begin{itemize}
	\item Completely decentralized MapReduce.
	\item My work could be used for a quick and intuitive way to solve embarrassingly parallel problems.
\end{itemize}

Unlike the current distributed applications which utilize distributed hash tables, we want to create a complete framework which can be used to build decentralized applications.
We have found no existing projects that provide a means of building your own DHT and DHT based applications.
...
\begin{description}
	...
	\item[Fault Tolerance]  Even in a network that is expected to remain static for long periods of time, the platform still has to deal with node failures.  
