...
%TODO: Intro
Distributed computing is the current trend and is the future trend.  
Here's some stats \cite{p2p-cloud}

Large, centralized environments are the norm for distributed computing.
These  large environments focus on using large amounts of homogeneous hardware.

One of the most popular distributed computing frameworks is MapReduce  \cite{mapreduce}.
...
These MapReduce platforms are highly centralized and tend to have single points of failure\cite{shvachko2010hadoop} as a result.
However, as we've previously noted, there are notable issues with a centralized design.

First and foremost is the issue of fault-tolerance \cite{p2p-cloud}
...
Many environments also anticipate a certain degree in homogenity in the system.
Finally deploying these systems and developing programs for them has an extremely steep learning curve.

There is no reason that these assumptions need to be the case for MapReduce, or distributed computing in general.
Moving away from the data center context allows us to 



There are a few significant reasons to use a dht
Elimination of metadata concerning a files location (but not necessarily other metadata)
Everybody knows who is responsible for what file and who needs to be a backup or help backup
heterogeneous
Alternatively, can be used to find and link  on demand resources.


Could use a processor scoring mechanism

Expansion on each of these in sections
...
Calculate some base score to yield a starting number of instances and a maximum.
Get to the maximum by automatic load balancing
