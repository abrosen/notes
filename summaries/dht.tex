...
\chapter{The Fourfathers}% Structured Overlays for Distributed Hash Tables
...
\subsection*{Lookup}
Each node maintains a routing table corresponding to their neighbors, those nodes it shares a face with.
Each hop forwards the lookup to the neighbor closest to the destination, until it comes to the responsible node.
This simple routing scheme uses only $2d$ space while giving average path length of $\frac{d}{4}\cdot n^{\frac{1}{d}}$, for a lookup time of $O(n^{\frac{1}{d}})$ hops\footnote{Around the same time CAN was being developed, Kleinberg was doing research into small world networks \cite{kleinberg2000small}.  He proved similar properties for lattice networks with a single shortcut.  What makes this network remarkable is lack of shortcuts.}.
...
Node $n$ informs $m$ that it is joining and they divide such that each becomes responsible for half.
...
\section{Tapestry}
Tapestry \cite{tapestry} is based off the same prefix-based lookup \cite{prr}  as Pastry \cite{pastry}.
...
\subsection*{Lookup}
Tapestry \cite{tapestry} implements a version  of the prefix-based lookup based \cite{prr}
