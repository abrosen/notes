...
%In one strategy nodes were given toke
...
The vast majority of DHTs choose a logarithmic sized routing table to provide a simple routing scheme that keeps the number of hops logarithmically bound to the network size.
Chapter \ref{chapter:background} discusses these tradeoffs in greater detail and how they affect the each DHT.


\subsection{Heterogeneity}
\label{subsec:hetero}
The mechanics behind load balancing assume nothing about the nature of the hardware it is running on.
From a theoretical perspective one node is the same as the other.
...
File sharing applications like Bittorrent are composed of many different computers, each running the same application
...
Distributed Hash Tables can provide all of these qualities to an application.
