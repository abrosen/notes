...
\begin{description}
	...
	The quality of fault-tolerance or \textit{robustness} \footnote{There is apparently a subtle difference between fault-tolerance and robustness, but we will use the two interchangeably here until we get told to stop.} means that the system still works even after a component breaks (or many component break).
...
It should be noted that there is not necessarily a hard and fast division between 

O ne question we are particulerIs there a way to do so at run time?


\subsection{How DHTs Address these Challenges}
%Without getting into the details of what a DHT is, what do they do?

Hash tables associate certain  cryptograpic keys with specific values.
This is done with Hash function.
Hash functions are designed to avoid Hash collision and the larger the keys, the less chance of a hash collision.
