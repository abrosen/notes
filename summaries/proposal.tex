...
Large frameworks are the norm, these frame works require a noisy amount of overhead to setup to obtain O(1) routing

MapReduce 
uses centralized framework
Very popular

These frameworks generally operate under datacenter assumptions and non-fault tolerant environments
homogenous

What about a framework that is heterogenous, self orgnaizing, and fault-tolerant nto in a data center assumptions
remove data center assumptions




There are a few significant reasons to use a dht
Elimination of metadata concerning a files location (but not nessicarily other metadata)
...
Exapansion on each of these in sections
...
Since we were ChordReduce on Amazon's EC2 and paying per instance per hour, we didn't use any more nodes than necessary.
